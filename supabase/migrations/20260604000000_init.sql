...
-- ============================================
-- Note: Recreating to ensure index utilization by forcing the optimizer 
-- to evaluate tenant_id filter BEFORE vector distance calculation.
...
LANGUAGE plpgsql
STABLE  -- Marking as STABLE helps the query planner
AS $$
BEGIN
    RETURN QUERY
    SELECT
    ...
    WHERE
        -- Critical: Force tenant filtering first for performance
        d.tenant_id = filter_tenant_id 
